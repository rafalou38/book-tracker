export class NumberSwiper {
			...
			function (numbers) {
				numbers.forEach(function (number) {
					if (number.isIntersecting === true) {
						...
						number.target.parentElement
							?.querySelector('.number-swiper-active-number')
							?.classList.remove('number-swiper-active-number');
	...
	getNumber(column, value) {
		...
		if (value == 'active') {
			return NS.el.querySelector(
				'.number-swiper-column-' + column + ' .number-swiper-active-number'
			);
		}
