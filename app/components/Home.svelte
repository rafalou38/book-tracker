<script lang="ts">
  ...
  async function actionWrapper(
    ...
    return action(message, cancelButtonText, actions);
  }

  let todos: { name: string }[] = [];
  let dones: { name: string }[] = []; //completed items go here
  const removeFromList = <T>(list: T[], item: T) =>
    list.filter((t) => t !== item);
  const addToList = <T>(list: T[], item: T) => [item, ...list];
  let textFieldValue = "";

  async function onItemTap(args: ItemEventData) {
    let result = await actionWrapper(
